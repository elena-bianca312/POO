        ...
        for (MyConsumer consumer : myConsumers) {
            if (!consumer.isBankrupt()) {
                ...
                if (consumer.getSkippedPay()) {
                    ...
                    int priceChangedDistributors = (int) (consumer.getNewBudget()
                            - consumer.getOldContractPrice() * PERCENTAGE);
                    //bankrupt
                    if (price < 0) {
                        //am adaugat doar acest if
                        if (consumer.getMonthNo() == 0
                                && consumer.getOldDistributor().equals(consumer.getDistributor())
                                && priceChangedDistributors > 0) {
                            consumer.setNewBudget(priceChangedDistributors);
